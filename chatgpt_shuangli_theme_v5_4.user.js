// ==UserScript==
// @name         ChatGPT 霜璃 · 完整冰晶龙娘主题 V5.4
// @namespace    https://chatgpt.com/
// @version      5.4.0
// @description  网页版霜璃完整主题 V5.4：升级网页伴侣交互，加入轻量鼠标视差、分状态台词、双击设置、完成庆祝与更稳定的最终状态显示。
// @match        https://chatgpt.com/*
// @match        https://www.chatgpt.com/*
// @run-at       document-end
// @grant        none
// @updateURL    https://raw.githubusercontent.com/z394547292-cloud/chatgpt-dragon-theme/main/chatgpt_shuangli_theme_v5_4.user.js
// @downloadURL  https://raw.githubusercontent.com/z394547292-cloud/chatgpt-dragon-theme/main/chatgpt_shuangli_theme_v5_4.user.js
// ==/UserScript==
...
(function () {
  ...
  function addInteractionStyle(){
    ...
    st.textContent=`
      ...
      @keyframes sl54Celebrate{
        ...
        16%{opacity:1}
        to{transform:translate3d(var(--cx),var(--cy),0) scale(1.15);opacity:0}
      }
  ...
  function addHeroHitbox(){
    ...
    hit.addEventListener('pointermove',e=>{
      if(get(KEY_INTERACTION,'1')==='0' || get(KEY_PARALLAX,'1')==='0')return;
      const r=hit.getBoundingClientRect();
      const nx=((e.clientX-r.left)/Math.max(1,r.width)-.5);
      const ny=((e.clientY-r.top)/Math.max(1,r.height)-.5);
      const hero=document.getElementById(HERO_ID);
      if(hero){
        hero.style.setProperty('--sl54-px',(nx*5).toFixed(2)+'px');
        hero.style.setProperty('--sl54-py',(ny*4).toFixed(2)+'px');
      }
    });
  ...
  function celebrateDone(){
    if(get(KEY_CELEBRATE,'1')==='0' || get(KEY_FX,'1')==='0')return;
    addCelebrateLayer();
  ...
  console.log('[霜璃主题] V5.4 loaded · transparent done state');
})();