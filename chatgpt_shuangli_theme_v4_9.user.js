// ==UserScript==
// @name         ChatGPT 霜璃 · 完整冰晶龙娘主题 V4.9
// @namespace    https://chatgpt.com/
// @version      4.9.0
// @description  网页版霜璃完整主题：强化四状态反馈——待机呼吸、思考冰环与粒子、输出龙息轨迹、完成高光与雪晶收束，并升级状态图管理器。
// @match        https://chatgpt.com/*
// @match        https://www.chatgpt.com/*
// @run-at       document-end
// @grant        none
// @updateURL    https://raw.githubusercontent.com/z394547292-cloud/chatgpt-dragon-theme/main/chatgpt_shuangli_theme_v4_9.user.js
// @downloadURL  https://raw.githubusercontent.com/z394547292-cloud/chatgpt-dragon-theme/main/chatgpt_shuangli_theme_v4_4.user.js
// ==/UserScript==
...
(function () {
  ...
  function syncStateImage(){
    const state=document.documentElement.getAttribute('data-sl-state')||'idle';
    const imgData=getStateImage(state);
  ...
  function openStateImageManager(){
    ...
    const row=(state,label)=>'<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 0;border-top:1px solid rgba(135,126,160,.10)"><div style="display:flex;align-items:center;gap:8px"><span>'+label+'</span><span data-status-for="'+state+'" style="padding:4px 7px;border-radius:999px;font-size:10px;background:rgba(238,240,246,.66);color:#8a8392">使用主图</span></div><div style="display:flex;gap:6px"><button class="sl42-btn" data-set="'+state+'">导入</button><button class="sl42-btn" data-clear="'+state+'">清除</button></div></div>';

    m.innerHTML=
      '<h3 style="margin:0 0 5px">❄ 霜璃多状态图片</h3>'+
      '<p style="margin:0 0 14px;color:#777184;font-size:12px;line-height:1.6">没有单独设置的状态会自动使用当前霜璃主图。</p>'+
      row('idle','待机')+
      row('thinking','思考')+
      row('generating','输出')+
      row('done','完成')+
    ...
    const labels={idle:'待机',thinking:'思考',generating:'输出',done:'完成'};
  ...
  function syncStateDialogue(state){
    ...
    if(bubble && stateDialogues[state]) bubble.textContent=stateDialogues[state];
  }
  ...
  function addSettings(){
    ...
    box.innerHTML=`
      <h3>❄ 霜璃主题设置</h3>
      <p>V4.9：强化四状态反馈，加入冰晶环、思考粒子、龙息轨迹、完成雪晶收束，并升级状态图片管理器。</p>
