// ==UserScript==
// @name         DeepSeek 霜璃 · 专属冰晶龙娘主题 V1.0
// @namespace    https://chat.deepseek.com/
// @version      1.0.0
// @description  DeepSeek 网页专属霜璃主题 V1.0：移植霜璃最终版视觉、五状态立绘、互动、安静模式与冰晶界面，并适配 DeepSeek 网页。
// @match        https://chat.deepseek.com/*
// @run-at       document-end
// @grant        none
// @updateURL    https://raw.githubusercontent.com/z394547292-cloud/chatgpt-dragon-theme/main/deepseek_shuangli_theme_v1_0.user.js
// @downloadURL  https://raw.githubusercontent.com/z394547292-cloud/chatgpt-dragon-theme/main/deepseek_shuangli_theme_v1_0.user.js
// ==/UserScript==
...
(function () {
  ...
  const DEEPSEEK_STYLE_ID = 'sl-ds-adapter-style';
  ...
  function detectState(){
    ...
    const busy=!!document.querySelector('[aria-busy="true"],[data-state="loading"],[data-loading="true"]');
    const text=(document.body?.innerText||'').slice(-7000);
    const thinking=/思考中|正在思考|深度思考|Thinking\.?\.?\.?|Reasoning/i.test(text);
    const generating=/生成中|正在生成|回答中|输出中|Generating|Responding/i.test(text);
  ...
  function hasConversation(){
    ...
    if(/\/a\/chat\/s\//.test(path)) return true;
    const main=document.querySelector('main,[role="main"]');
    if(!main) return false;
    const blocks=[...main.querySelectorAll('div,p')].filter(el=>{
  ...
  function addWelcome(){
    ...
    w.id=WELCOME_ID;
    w.innerHTML=
      '<div class="eyebrow">SHUANGLI · DEEPSEEK</div>'+
      '<h1>霜璃在这里。</h1>'+
  ...
  function addPolishStyle(){
    ...
    st.textContent=`
      ...
      html,body{
        background:
          radial-gradient(circle at 84% 12%,rgba(201,187,230,.22),transparent 28%),
          radial-gradient(circle at 12% 88%,rgba(184,211,234,.18),transparent 32%),
          linear-gradient(145deg,#f7f8fc 0%,#eef2f8 52%,#e9e5f1 100%)!important;
  ...
  function addDeepSeekAdapterStyle(){
    if(document.getElementById(DEEPSEEK_STYLE_ID))return;
    const st=document.createElement('style');
    st.id=DEEPSEEK_STYLE_ID;
    st.textContent=`
      html,body,#root{min-height:100%!important;background:transparent!important}

      #chat-input,textarea,div[contenteditable="true"]{
        caret-color:#7f6eb0!important;
      }

      #chat-input:focus,textarea:focus,div[contenteditable="true"]:focus{
        outline:none!important;
      }
      ...
      :is(aside,nav){
        background:
          radial-gradient(circle at 85% 8%,rgba(202,188,234,.12),transparent 28%),
          rgba(242,246,252,.86)!important;
        border-right:1px solid rgba(135,124,166,.14)!important;
        backdrop-filter:blur(22px) saturate(116%)!important;
        -webkit-backdrop-filter:blur(22px) saturate(116%)!important;
      }

      :is(aside,nav) :is(a,button){
        border-radius:12px!important;
        transition:.18s ease!important;
      }

      :is(aside,nav) :is(a,button):hover{
        background:linear-gradient(90deg,rgba(219,226,241,.58),rgba(232,226,244,.50))!important;
      }

      :is(main,[role="main"]) :is(p,li){
        color:#403b4a;
      }

      :is(div,form):has(> #chat-input),
      :is(div,form):has(> textarea),
      :is(div,form):has(> div[contenteditable="true"]){
        border-color:rgba(137,126,169,.20)!important;
      }

      #${HERO_ID}{z-index:2147482500!important}
      :is([role="dialog"],[role="menu"]){z-index:2147483600!important}
    `;
    (document.head||document.documentElement).appendChild(st);
  }
  ...
  function positionHeroSpeech(){
    ...
    const composer=document.querySelector('div:has(> #chat-input), form:has(#chat-input), form > div:has(textarea), form > div:has([contenteditable="true"]), [data-composer-surface-variant], .composer-surface-chrome');
    if(composer){
  ...
  function markTaskEntry(){ return false; }

  function detectTaskPage(){
    document.documentElement.setAttribute('data-sl-task-page','0');
  }
  ...
  function addSettings(){
    ...
    box.innerHTML=`
      <h3>❄ 霜璃主题设置</h3>
      <p>DeepSeek 专属版 V1.0：适配 chat.deepseek.com，保留霜璃最终版视觉、五状态、互动和安静模式。</p>
  ...
  console.log('[霜璃主题] DeepSeek V1.0 loaded · dedicated adapter');
})();